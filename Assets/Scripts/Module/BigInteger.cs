...
namespace Function
{
    ...
    internal class DigitsArray
    {
        ...
        internal DType this[int index]
        {
            get
            {
                if (index < DataUsed) return m_data[index];
        ...
        internal bool IsZero => DataUsed == 0 || (DataUsed == 1 && m_data[0] == 0);
        ...
        {
            var result = "";
            foreach (var data in m_data) result += data + " ";
            return result;
        }
        ...
        internal void ResetDataUsed()
        {
            ...
            if (IsNegative)
            {
                while (DataUsed > 1 && m_data[DataUsed - 1] == AllBits) --DataUsed;
                DataUsed++;
            }
            else
            {
                while (DataUsed > 1 && m_data[DataUsed - 1] == 0) --DataUsed;
                if (DataUsed == 0) DataUsed = 1;
            }
        }
        ...
        {
            var temporary = new List<DType>(m_data);
            var shiftAmount = DataSizeBits;
    ...
    public class BigInteger
    {
        ...
        public string ChangeMoney()
        {
            ...
            if (index < unit.Length)
            {
                ...
                if (index > 0)
                {
                    var r = cVal[index] * 1000 + cVal[index - 1];
                    return string.Format("{0:#.###}{1}", r / 1000f, unit[index]);
        ...
        private void ConstructFrom(byte[] array, int offset, int length)
        {
            ...
            if (offset > array.Length || length > array.Length) throw new ArgumentOutOfRangeException("offset");
            if (length > array.Length || offset + length > array.Length)
                throw new ArgumentOutOfRangeException("length");

            var estSize = length / 4;
            var leftOver = length & 3;
            if (leftOver != 0) ++estSize;
        ...
        private void Construct(string digits, int radix)
        {
            ...
            for (var idx = digits.Length - 1; idx >= nDigits; idx--)
            {
                ...
                if (d >= radix) throw new ArgumentOutOfRangeException("digits");
                result += multiplier * d;
                multiplier *= radix;
            }
        ...
        public static BigInteger operator -(BigInteger leftSide)
        {
            ...
            if (leftSide.IsZero) return new BigInteger(0);

            var da = new DigitsArray(leftSide.m_digits.DataUsed + 1, leftSide.m_digits.DataUsed + 1);

            for (var i = 0; i < da.Count; i++) da[i] = ~leftSide.m_digits[i];
        ...
        {
            if (ReferenceEquals(leftSide, null)) throw new ArgumentNullException("leftSide");
            if (leftSide.IsNegative) return -leftSide;
            return leftSide;
        }
        ...
        public static BigInteger operator *(BigInteger leftSide, BigInteger rightSide)
        {
            ...
            for (var i = 0; i < leftSide.m_digits.DataUsed; i++)
            {
                ...
                if (carry != 0) da[i + rightSide.m_digits.DataUsed] = (DType)carry;
            }
        ...
        public static BigInteger operator /(BigInteger leftSide, BigInteger rightSide)
        {
            ...
            if (rightSide.IsZero) throw new DivideByZeroException();
            ...
            if (leftSide < rightSide) return new BigInteger(0);
            ...
            out BigInteger remainder)
        {
            ...
            var d = 0;
            for (var mask = DigitsArray.HiBitSet; mask != 0 && (val & mask) == 0; mask >>= 1) d++;

            var remainderLen = leftSide.m_digits.DataUsed + 1;
            var remainderDat = new DType[remainderLen];
            ...
            var result = new DType[leftSide.m_digits.Count + 1];
            var resultPos = 0;
            ...
            for (int j = remainderLen - rightSide.m_digits.DataUsed, pos = remainderLen - 1; j > 0; j--, pos--)
            {
                ...
                {
                    if (qHat == carryBit ||
                        qHat * secondDivisor > (rHat << DigitsArray.DataSizeBits) + remainderDat[pos - 2])
                    {
                        qHat--;
                        rHat += firstDivisor;
                        if (rHat < carryBit) continue;
                    }

                    break;
                }

                for (var h = 0; h < divisorLen; h++) dividendPart[divisorLen - h - 1] = remainderDat[pos - h];
            ...
            out BigInteger remainder)
        {
            ...
            var result = new DType[leftSide.m_digits.Count];
            ...
            var j = 0;
            for (var i = quotientDigits.DataUsed - 1; i >= 0; i--, j++) quotientDigits[j] = result[i];
        ...
        public static BigInteger operator &(BigInteger leftSide, BigInteger rightSide)
        {
            ...
            for (var idx = 0; idx < len; idx++) da[idx] = leftSide.m_digits[idx] & rightSide.m_digits[idx];
            return new BigInteger(da);
        }
        ...
        public static BigInteger operator ~(BigInteger leftSide)
        {
            ...
            for (var idx = 0; idx < da.Count; idx++) da[idx] = ~leftSide.m_digits[idx];
        ...
        public static BigInteger operator >> (BigInteger leftSide, int shiftCount)
        {
            ...
            {
                for (var i = da.Count - 1; i >= da.DataUsed; i--) da[i] = DigitsArray.AllBits;

                var mask = DigitsArray.HiBitSet;
                for (var i = 0; i < DigitsArray.DataSizeBits; i++)
                {
                    if ((da[da.DataUsed - 1] & mask) == DigitsArray.HiBitSet) break;
        ...
        {
            if (ReferenceEquals(leftSide, rightSide)) return 0;
        ...
        public static bool operator ==(BigInteger leftSide, BigInteger rightSide)
        {
            ...
            if (leftSide.IsNegative != rightSide.IsNegative) return false;
        ...
        public static bool operator >(BigInteger leftSide, BigInteger rightSide)
        {
            ...
            if (leftSide.IsNegative != rightSide.IsNegative) return rightSide.IsNegative;
        ...
        public static bool operator <(BigInteger leftSide, BigInteger rightSide)
        {
            ...
            if (leftSide.IsNegative != rightSide.IsNegative) return leftSide.IsNegative;
        ...
        public override bool Equals(object obj)
        {
            ...
            if (m_digits.DataUsed != c.m_digits.DataUsed) return false;
        ...
        public string ToString(int radix)
        {
            ...
            if (radix == 10 && negative) return "-" + result;
        ...
        public string ToHexString()
        {
            ...
            for (var i = m_digits.DataUsed - 2; i >= 0; i--) sb.AppendFormat(f, m_digits[i]);
