...
namespace Creature.Data
{
    ...
    {
        [Header("[Summon Level]")] [Header("[초기 레벨, 초기 경험치, 레벨 당 경험치 증가율]")]
        ...
        public int MaxGearLevel { get; }
        public float MaxGearExp { get; private set; }

        public void InitializeData()
        {
            CurrentSquadLevel = ES3.Load($"{Enum.LevelType.CurrentLv}{Enum.SummonEquipmentType.Squad}CurrentLevel",
                LevelFirstValue);
            CurrentSquadExp = ES3.Load<float>($"{Enum.LevelType.CurrentExp}{Enum.SummonEquipmentType.Squad}CurrentExp",
                ExpFirstValue);

            CurrentWeaponLevel = ES3.Load($"{Enum.LevelType.CurrentLv}{Enum.SummonEquipmentType.Weapon}CurrentLevel",
                LevelFirstValue);
            CurrentWeaponExp =
                ES3.Load<float>($"{Enum.LevelType.CurrentExp}{Enum.SummonEquipmentType.Weapon}CurrentExp",
                    ExpFirstValue);

            CurrentGearLevel = ES3.Load($"{Enum.LevelType.CurrentLv}{Enum.SummonEquipmentType.Gear}CurrentLevel",
                LevelFirstValue);
            CurrentGearExp = ES3.Load<float>($"{Enum.LevelType.CurrentExp}{Enum.SummonEquipmentType.Gear}CurrentExp",
                ExpFirstValue);
        ...
        private void UpdateCurrentExp(Enum.SummonEquipmentType type, int currentExpIncreaseValue)
        {
            switch (type)
            {
                case Enum.SummonEquipmentType.Squad:
                    ...
                    if (CurrentSquadExp >= MaxSquadExp && CurrentSquadLevel < MaxLevelFirstValue)
                        OnLevelUpgrade?.Invoke(type);
                    ES3.Save($"{Enum.LevelType.CurrentExp}{Enum.SummonEquipmentType.Squad}CurrentExp",
                        CurrentWeaponExp);
                    break;
                case Enum.SummonEquipmentType.Weapon:
                    CurrentWeaponExp += currentExpIncreaseValue;
                    if (CurrentWeaponExp >= MaxWeaponExp && CurrentWeaponLevel < MaxLevelFirstValue)
                        OnLevelUpgrade?.Invoke(type);
                    ES3.Save($"{Enum.LevelType.CurrentExp}{Enum.SummonEquipmentType.Weapon}CurrentExp",
                        CurrentWeaponExp);
                    break;
                case Enum.SummonEquipmentType.Gear:
                    CurrentGearExp += currentExpIncreaseValue;
                    if (CurrentGearExp >= MaxGearExp && CurrentGearLevel < MaxLevelFirstValue)
                        OnLevelUpgrade?.Invoke(type);
        ...
        private void UpdateLevel(Enum.SummonEquipmentType type)
        {
            switch (type)
            {
                case Enum.SummonEquipmentType.Squad:
                    ...
                    ES3.Save($"{Enum.LevelType.CurrentLv}{Enum.SummonEquipmentType.Squad}CurrentLevel",
                        CurrentSquadLevel);
                    break;
                case Enum.SummonEquipmentType.Weapon:
                    CurrentWeaponLevel++;
                    ...
                    ES3.Save($"{Enum.LevelType.CurrentLv}{Enum.SummonEquipmentType.Weapon}CurrentLevel",
                        CurrentWeaponLevel);
                    break;
                case Enum.SummonEquipmentType.Gear:
                    CurrentGearLevel++;
                    ...
                    ES3.Save($"{Enum.LevelType.CurrentLv}{Enum.SummonEquipmentType.Gear}CurrentLevel",
                        CurrentGearLevel);
                    break;
            }
